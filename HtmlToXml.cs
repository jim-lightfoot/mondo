...
namespace Mondo.Web
{
    ...
    public class HtmlToXml 
    {
        ...
        public class Element : Node
        {
            ...
            public override bool Write(HtmlToXml objParser, StringBuilder sb, int iLevel)
            {
                ...
                if(m_aChildren.Count > 0)
                {
                    ...
                    if(m_objParser.RemoveEmpties && bEmpty)
                        return(false);
        ...
        {
            if(m_iIndex + strValue.Length >= m_iLength)
                return(false);

            return(m_strInput.Substring(m_iIndex, strValue.Length) == strValue);
        }
        ...
        private bool ProcessBeginEnd(string strStart, string strEnd, bool bRemove)
        {
            if(CompareNext(strStart))
            {
                ...
                {
                    if(!bRemove)
                        AddToParent(new Literal(m_strInput.Substring(m_iIndex) + strEnd));
